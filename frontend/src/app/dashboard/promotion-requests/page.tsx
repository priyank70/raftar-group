...
import { Shield, CheckCircle, XCircle, AlertCircle, UserCheck } from 'lucide-react';
...
export default function PromotionRequestsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['promotion-requests', page],
    queryFn: () => api.get(`/users/promotion-requests?page=${page}&limit=10`).then((r) => r.data),
  ...
  const voteMutation = useMutation({
    mutationFn: ({ id, vote, comment }: { id: string; vote: 'approve' | 'reject'; comment?: string }) =>
      api.put(`/users/promotion-requests/${id}/vote`, { vote, comment }),
    onSuccess: () => {
      toast.success('Vote recorded!');
      qClient.invalidateQueries({ queryKey: ['promotion-requests'] });
      ...
      <div className="card p-5 bg-gradient-to-br from-primary-900 to-primary-700 text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-primary-200 mt-1">Review and vote on admin promotions. Requires approval from all active members.</p>
          </div>
          <Shield className="w-8 h-8 text-accent-light opacity-80" />
        </div>
      </div>
      ...
      <div className="space-y-4">
        {requests.length === 0 ? (
          <div className="text-center p-8 bg-white rounded-2xl border border-gray-100">
            ...
            <p className="text-xs text-muted mt-1">There are no pending promotion requests.</p>
          </div>
        ) : (
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-primary-100 flex items-center justify-center font-bold text-primary-700">
                      {req.candidateId?.name?.charAt(0) || 'C'}
                    </div>
                    <div>
                      <p className="font-semibold text-gray-900 text-sm">Promote: {req.candidateId?.name}</p>
                      <p className="text-xs text-muted">
                        Requested by {req.requestedBy?.name} · {formatDate(req.createdAt)}
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-gray-50 rounded-xl p-4 mb-4 text-sm border border-gray-100 space-y-3">
                  {req.reason && (
                    <div>
                      <p className="text-muted mb-1 text-xs font-semibold uppercase">Reason:</p>
                      <p className="font-medium text-gray-900 bg-white p-2 rounded border border-gray-100 italic">
                        "{req.reason}"
                      </p>
                    </div>
                  )}
                  ...
                  <div className="border-t pt-3 mt-3">
                    <p className="text-muted mb-2 text-xs font-semibold uppercase">Votes Detail:</p>
                    <div className="space-y-1.5">
                      {req.votes.length === 0 ? (
                        <p className="text-xs text-muted">No votes yet</p>
                      ) : req.votes.map((v: any, index: number) => (
                        <div key={index} className="flex justify-between items-center text-xs bg-white p-2 rounded border border-gray-100">
                          <span className="font-medium">{v.userId?.name || 'User'}</span>
                          <div className="flex items-center gap-2">
                            ...
                            {v.comment && <span className="text-muted font-normal italic">({v.comment})</span>}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="flex items-center justify-between mt-4">
                  ...
                  {req.status === 'pending' && req.candidateId?._id !== user?._id && (
                    user?.isDisabled ? (
